...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        elif data == "feed_birds":   # 新增喂养功能
            if user['feed'] >= 10:
                user['feed'] -= 10
                exp_gain = random.randint(40, 60)
                leveled_up = add_exp(update.effective_user.id, exp_gain)
                await query.message.reply_text(
                    f"🌾 喂养成功！消耗 10 鸟粮\n获得 **{exp_gain}** 经验值！"
                )
                if leveled_up:
                ...
                await query.message.reply_text("❌ 鸟粮不足！至少需要 10 鸟粮才能喂养")
...
def main():
    ...
    logger.info("🚀 飞鸟牧场机器人启动成功！（新增喂养功能）")
